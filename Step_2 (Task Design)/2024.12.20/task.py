...
class CardiacSyncedLearningTask:
    ...
    def wait_for_timing(self, target_time, window=0.005):
        ...
        # Coarse wait with a timeout
        timeout = time.time() + 5.0  # 5-second maximum wait
        while (target_time - self.get_lsl_time()) > window:
            if time.time() > timeout:
                print(f"Timeout exceeded while waiting for target time: {target_time:.6f}")
                return
            core.wait(0.001)  # Allow the system to process other tasks

        # Fine-tuning loop with additional safety checks
    ...
    def run_trial(self, trial_num, n_trials, is_reversed, block_outcomes, current_block, reversal_point):
        ...
        This optimized version calculates cardiac timing requirements in advance and uses 
        the first valid R-peak that satisfies both minimum delay and cardiac timing 
        requirements, significantly reducing the delay between choice and feedback.
        ...
        if response_made:
            ...
            try:
                ...
                if self.current_rr_average is not None:
                    ...
                    print(f"Calculated cardiac delay: {cardiac_delay*1000:.1f}ms")

                    self.prepare_feedback(feedback_type)

                    minimum_presentation_time = choice_time + self.settings['delay_min']
                    got_timing = False
                    ...
                    while not got_timing and (time.time() - timeout_start) < 5.0:
                        if not self.r_peak_times.empty():
                            ...
                            potential_presentation_time = peak_time + cardiac_delay - self.settings['monitor_refresh_delay']

                            if potential_presentation_time >= minimum_presentation_time:
                                intended_presentation_time = potential_presentation_time
                    ...
                    if got_timing:
                        print(f"Proceeding with target time {intended_presentation_time:.6f}")
                        self.wait_for_timing(intended_presentation_time, window=0.002)

                        pre_flip_time = self.get_lsl_time()
                        if pre_flip_time >= intended_presentation_time - 0.002:
                            ...
                            print("Warning: Target time skipped, displaying feedback immediately.")
                            self.win.flip()
                    else:
                        print("Warning: No valid timing found. Skipping cardiac synchronization.")
