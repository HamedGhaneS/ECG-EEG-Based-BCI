...
class CardiacSyncedLearningTask:
    ...
    * Press YELLOW BUTTON to BEGIN the task * """
    ...
    def collect_r_peaks(self):
        ...
        while True:
            try:
                sample, _ = self.inlet.pull_sample(timeout=0.05)  # Ignore the sender's timestamp

                if sample is not None:
                    local_time = self.get_lsl_time()  # Use local clock instead
                    peak_counter += 1
                    ...
                    if (last_peak_time is None or 
                        ...
                        try:
                            self.r_peak_times.put_nowait(local_time)
