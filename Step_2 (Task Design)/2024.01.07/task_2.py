...
class CardiacSyncedLearningTask:
    ...
    def update_rr_average(self, new_peak_time):
        """Update running R-R interval average with the new peak."""
    ...
    def collect_r_peaks(self):
        """Continuously collect R-peak markers with improved reliability and validation."""
        last_peak_time = None
        missed_peaks_count = 0
        peak_counter = 0  # Counter for tracking total peaks received
        ...
        while True:
            try:
                ...
                if sample is not None:
                    lsl_time = timestamp  # Use LSL timestamp exclusively
                    peak_counter += 1

                    # Log every R-peak with detailed timing info
                    print(f"R-peak #{peak_counter} received at LSL time: {lsl_time:.6f}")

                    if last_peak_time is not None:
                        interval = lsl_time - last_peak_time
                    ...
                    self.update_rr_average(lsl_time)

                    # Store peak time in queue if it's valid
                    if (last_peak_time is None or 
                        (self.settings['rr_interval_min'] <= (lsl_time - last_peak_time) <= self.settings['rr_interval_max'])):

                        # Check queue size before adding
                        if self.r_peak_times.qsize() >= self.settings['rpeak_queue_size']:
                            # Remove oldest peak if queue is full
                            try:
                                _ = self.r_peak_times.get_nowait()
                                print("Removed oldest peak from queue (maximum size reached)")
                            except queue.Empty:
                                pass

                        # Add new peak to queue
                        try:
                            self.r_peak_times.put_nowait(lsl_time)
                            print(f"Peak #{peak_counter} added to queue. Queue size: {self.r_peak_times.qsize()}")
                        except queue.Full:
                            print("Warning: R-peak queue is full")

                    last_peak_time = lsl_time

                else:
                    # No sample received, wait a bit
                    time.sleep(0.001)
    ...
    def run_trial(self, trial_num, n_trials, is_reversed, block_outcomes, current_block, reversal_point):
        """Run a single trial with optimized cardiac synchronization and pre-calculated delays."""
        ...
        if response_made:
            ...
            # Pre-calculate cardiac timing parameters during fixation
            if self.current_rr_average is not None:
                ...
                print("\nPre-calculated timing parameters:")
                print(f"Current R-R average: {ave_rr:.3f}s")
                print(f"Block condition: {phase}-{timing_point}")
                print(f"Target timing: {rr_percentage*100:.1f}% of R-R")
                print(f"Pre-calculated cardiac delay: {cardiac_delay*1000:.1f}ms")

            # Start post-choice fixation period
            self.stim['fixation'].draw()
            self.win.flip()
            ...
            # Wait exactly 1 second for fixation
            ...
            # Prepare feedback stimulus during fixation
            self.prepare_feedback(feedback_type)

            try:
                # Clear queue after fixation and wait for next valid R-peak
                while not self.r_peak_times.empty():
                    _ = self.r_peak_times.get()

                # Keep displaying fixation while waiting for R-peak
                print("\nWaiting for first valid R-peak after fixation...")
                timeout_start = self.get_lsl_time()
                got_timing = False

                while not got_timing and (self.get_lsl_time() - timeout_start) < 3.0:  # 3 second timeout
                    if not self.r_peak_times.empty():
                        peak_time = self.r_peak_times.get()
                        if peak_time > fixation_end_time:
                            ...
                            print(f"Target presentation time: {intended_presentation_time:.6f}")
                            got_timing = True
                            self.win.flip()  # Clear fixation immediately
                            break
                        else:
                ...
                if got_timing:
                    # Wait precisely for the intended presentation time
                    current_time = self.get_lsl_time()
                    if intended_presentation_time > current_time:
                        remaining_time = intended_presentation_time - current_time
                        print(f"Waiting {remaining_time*1000:.1f}ms for target presentation...")
