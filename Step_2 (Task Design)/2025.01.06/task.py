...
class CardiacSyncedLearningTask:
    ...
    def collect_r_peaks(self):
        ...
        This version ensures proper queuing of R-peaks and accurate timing synchronization."""
        last_peak_time = None
        ...
        while True:
            try:
                ...
                if sample is not None:
                    # Important: Use LSL timestamp exclusively
                    lsl_time = timestamp  # Don't convert to system time

                    if last_peak_time is not None:
                        interval = lsl_time - last_peak_time
                        if interval < self.settings['rr_interval_min']:
                            print(f"Warning: Suspiciously short R-R interval: {interval:.3f}s")
                            continue
                    ...
                    # Store only LSL timestamp in queue
                    self.r_peak_times.put(lsl_time)

                    # Keep queue size manageable
                    while self.r_peak_times.qsize() > self.settings['rpeak_queue_size']:
       ...
       if response_made:
           ...
           try:
               ...
               if self.current_rr_average is not None:
                   ...
                   print(f"Fixation started at: {fixation_start:.6f}")
                   ...
                   if got_timing:
                       ...
                       self.wait_for_timing(intended_presentation_time, window=0.005)
