...
def process_and_plot(epochs_dict, channels, ax, title, condition_info, is_single_peak=False, peak_type=None):
    ...
    if is_single_peak:
        ...
        if peak_type == 'Outcome':
            ...
            for peak in ['R1', 'R2', 'R3']:
                ...
                # Pick channels before baseline correction
                peak_epochs.pick(channels)
                # Apply baseline correction to all epochs at once
                peak_epochs.apply_baseline(baseline=(-0.2, -0.05))
                # Now separate conditions and get data
                cond1_epochs = peak_epochs[event_names[0]]
                cond2_epochs = peak_epochs[event_names[1]]
                # Average across channels for each condition
                cond1_data = np.mean(cond1_epochs.get_data(), axis=1)  # Average across channels
                cond2_data = np.mean(cond2_epochs.get_data(), axis=1)  # Average across channels
                all_peak_data.append((cond1_data, cond2_data))
            
            # Average across peaks
            cond1_data = np.mean([data[0] for data in all_peak_data], axis=0)
            cond2_data = np.mean([data[1] for data in all_peak_data], axis=0)
    ...
    # Apply baseline correction to all epochs before separating conditions
    epochs.apply_baseline(baseline=(-0.2, -0.05))
    
    # Now separate conditions and average across channels
    cond1_epochs = epochs[event_names[0]]
    cond2_epochs = epochs[event_names[1]]
    cond1_data = np.mean(cond1_epochs.get_data(), axis=1)  # Average across channels
    cond2_data = np.mean(cond2_epochs.get_data(), axis=1)  # Average across channels
    times = epochs.times
...
def process_and_plot_vep(epochs_dict, channels, ax, title, lock_type, invert=False):
    ...
    mean_across_trials = data.mean(axis=0)
    mean_data = mean_across_trials.mean(axis=0)
    if invert:
        mean_data = -mean_data
    sem_data = stats.sem(mean_across_trials, axis=0)
    times = epochs.times
        ...
        try:
            ...
            for condition_name, condition_info in analysis_conditions.items():
                ...
                if condition_name == "vep":
                    ...
                    # Plot combined analyses - only for HEP electrode groups
                    ...
                    # Plot individual R-peak analyses - only for HEP electrode groups
                    for i, group_name in enumerate(HEP_GROUPS):
